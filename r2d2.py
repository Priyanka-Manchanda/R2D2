...
if __name__ == "__main__":
    ...
    def command(text):
        ...
        for row in cursor.execute("SELECT * FROM Movies WHERE title LIKE ? ORDER BY rating",('%'+text+'%')):
    ...
    cursor.execute("""CREATE TABLE Movies
                  (title text, cast text, release_date text, 
                   rating text""")
               
    albums = [('The Shawshank Redemption (1994)', 'Andy Hunter', '7/9/2002', '9.2'),
          ('The Godfather (1972)', 'Red', '2/1/2011', '9.2'),
          ('12 Angry Men', 'Thousand Foot Krutch', '4/17/2012', '9.0'),
          ('The Good Life', 'Trip Lee', '4/10/2012', '9.0')]
    cursor.executemany("INSERT INTO Movies VALUES (?,?,?,?,?)", albums)
    conn.commit()
    '''SearchBox(root, command=command, placeholder="Type and press enter", entry_highlightthickness=0).pack(pady=6, padx=3)
    root.title("imdb.com");'''
